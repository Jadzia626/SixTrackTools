...
class Fort2():
    ...
    def loadFile(self):
        ...
        with open(fort2File,"r") as inFile:
            ...
            for theLine in inFile:
                ...
                if whatStage == 1:
                    ...
                    if len(lineElems) == 1:
                        if lineElems[0] == "MULTICOL":
                            self.isMultiCol = True
                            continue
                    if self.isMultiCol:
                        if len(lineElems) >= 3:
                            self.structDataM["Full"].append(lineElems[0])
                            self.structDataM["Name"].append(lineElems[1])
                            self.structDataM["SPos"].append(lineElems[2])
                        else:
                            logger.warning("Line %d has the wrong number of elements" % whatLine)
                    else:
                        for e in range(len(lineElems)):
                            self.structDataS.append(lineElems[e])
        ...
        self.structDataS         = np.asarray(self.structDataS[:],     dtype="str")
        self.structDataM["Full"] = np.asarray(self.structDataM["Full"],dtype="str")
        self.structDataM["Name"] = np.asarray(self.structDataM["Name"],dtype="str")
        self.structDataM["SPos"] = np.asarray(self.structDataM["SPos"],dtype="float")
            ...
            if self.isMultiCol:
                ...
                for e in range(len(self.structDataM["Full"])):
                    ...
                    outFile.write(("{:<24s} {:<24s} {:17.9f}\n").format(
                        ...
                        self.structDataM["SPos"][e],
                    ))
            else:
                for e in range(len(self.structDataS)):
                    outFile.write(("{:<24s} ").format(self.structDataS[e]))
                    if e > 0 and e%3 == 2:
                        outFile.write("\n")
                if not e%3 == 2:
                    outFile.write("\n")
